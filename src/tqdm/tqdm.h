...
namespace tqdm {
    ...
    private:
        ...
        void hsv_to_rgb(float h, float s, float v, int &r, int &g, int &b) {
            ...
            int p = static_cast<int>(255.0f * (v * (1.0f - s)));
            int q = static_cast<int>(255.0f * (v * (1.0f - s * f)));
            int t = static_cast<int>(255.0f * (v * (1.0f - s * (1.0f - f))));
            v *= 255;
            i %= 6;
    ...
    public:
        ...
        void progress(int curr, int tot) {
            if (is_tty && (curr % period == 0)) {
                ...
                double dt = static_cast<std::chrono::duration<double>>(now - t_old).count();
                double dt_tot = static_cast<std::chrono::duration<double>>(now - t_first).count();
                int dn = curr - n_old;
                n_old = curr;
                t_old = now;
                ...
                if (use_ema) {
                    ...
                    for (unsigned int i = 1; i < deq_t.size(); i++) {
                        ...
                        avgrate = static_cast<double>(alpha_ema) * r + (1.0 - static_cast<double>(alpha_ema)) * avgrate;
                    }
                } else {
                    double dtsum = std::accumulate(deq_t.begin(), deq_t.end(), 0.);
                    int dnsum = static_cast<int>(std::accumulate(deq_n.begin(), deq_n.end(), 0.));
                    avgrate = dnsum / dtsum;
                }
                ...
                double pct = static_cast<double> (curr) / (static_cast<double>(tot) * 0.01);
                if ((tot - curr) <= period) {
                    pct = 100.0;
                    avgrate = tot / dt_tot;
                    curr = tot;
                ...
                double fills = static_cast<double> (curr) / tot * width;
                ...
                if (use_colors) {
                    if (color_transition) {
                        ...
                        hsv_to_rgb(static_cast<float>(0.0 + 0.01 * pct / 3.0), 0.65f, 1.0f, r, g, b);
                ...
                if (!in_screen and (curr != tot)) printf("%s", bars[static_cast<size_t> (8.0 * (fills - ifills))]);
